...
export default class Login extends Component {
    ...
    handleSubmit(event) {
        ...
        { withCredentials: true }).then(response => { if (response.data.status === 'created') {
            console.log("You can come in ...")
            this.props.handleSuccessfulAuth()
        } else {
            this.setState({errorText: "Wrong Email or Password try again!"})
            this.props.handleUnSuccessfulAuth()
        }
    })
        .catch(error => { this.setState({ errorText: "An error occured" })})
